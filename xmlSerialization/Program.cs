...
namespace xmlSerialization
{
    class Program
    {
        ...
        private static void SerializeTeacherUsingFile()
        {
            ...
            using (FileStream fs = new FileStream("c:/temp/teacher.txt", FileMode.Create))
            {
                teacherSerializer.Serialize(fs, Teacher1);
                fs.Flush();
            }

            using (FileStream fs = new FileStream("c:/temp/teacher.txt", FileMode.Open))
            {
        ...
        private static void SerializeStudentUsingFile()
        {
            ...
            using (FileStream fs = new FileStream("c:/temp/student.txt", FileMode.Create))
            {
                studentSerializer.Serialize(fs, student1);
                fs.Flush();
            }

            using (FileStream fs = new FileStream("c:/temp/student.txt", FileMode.Open))
            {
